...
import org.springframework.web.bind.annotation.PostMapping; // BU EKLENDİ
import org.springframework.web.bind.annotation.RequestParam; // BU EKLENDİ
...
public class MesajKontrol {
    ...
    // --- KONUM PAYLAŞ / GİZLE ---
    @PostMapping("/konum-guncelle")
    @ResponseBody // Sayfa yenilenmesin, arka planda halletsin
    public String konumGuncelle(@RequestParam(required = false) Double enlem, 
                                @RequestParam(required = false) Double boylam, 
                                Principal principal) {
        Ogrenci ben = ogrenciServisi.mevcutOgrenciyiGetir(principal.getName());
        ogrenciServisi.konumGuncelle(ben, enlem, boylam);
        return "OK";
    }
}